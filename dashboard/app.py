...
with col1:
    amount_paid = st.number_input(
        "Amount Paid", min_value=0.0, value=50000.0, step=100.0
    )
    amount_received = st.number_input(
        "Amount Received", min_value=0.0, value=50000.0, step=100.0
    )
...
with col2:
    ...
    sender_tx_count = st.number_input(
        "Sender's Historical Transaction Count", min_value=0, value=450, step=1
    )
    sender_avg_amount = st.number_input(
        "Sender's Average Transaction Amount", min_value=0.0, value=500.0, step=50.0
    )
    threshold = st.slider(
...
if st.button("🔍 Analyze Transaction", type="primary", use_container_width=True):
    ...
    # ── SHAP reasons as bar chart ───────────────────────────────
    # Default: show top 4 (aligned with Regulation B / ECOA adverse
    # action notice standard — disclosing more than 4 principal reasons
    # is not shown to help the end user). Expandable up to 10 for
    # compliance officers / analysts who want deeper detail.
    all_reasons = result["top_reasons"]
    show_all = st.checkbox(
        f"View more reasons (showing 4 of {len(all_reasons)} by default)"
    )
